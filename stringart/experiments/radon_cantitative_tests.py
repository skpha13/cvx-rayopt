...
def plot_input_output_grid(images, solutions, labels=None, max_cols=5):
    ...
    num_rows = (num_images + max_cols - 1) // max_cols  # ceil division
...
def process_image(image_path: Path) -> np.ndarray:
    ...
    A, x, _ = solver.radon(patience=10)
    solution = solver.compute_solution(A, x, uds=True)
...
def main():
    with Pool(cpu_count()) as pool:
        solutions = pool.map(process_image, image_paths)
